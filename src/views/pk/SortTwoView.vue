...
onMounted(async () => {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      //挂载的时候获取students的值
      console.log("判断是否打分");
      window.localStorage.setItem("pdtwo", response.data);
...
const zancun = () => {
  ...
  window.localStorage.setItem("pdzancun2", true);
};
...
const onsubmit = () => {
  ...
  //判断是否重复打分
  // zancun
  if (window.localStorage.getItem("pdtwo") == "0") {
    var flag = 0;
    console.log("提交前修改");
    console.log(state.students);
    for (let i = 0; i < state.students.length; i++) {
    ...
    window.localStorage.setItem("pdtwo", 1);
    if (flag == 0) {
      ElMessage({
        message: "上传成功",
        type: "success",
