...
onMounted(async () => {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      //挂载的时候获取students的值
      console.log("判断是否打分");
      window.localStorage.setItem("pdfour", response.data);
  ...
  console.log(inputs);
  //先获取各个选手的名字学院
  if (window.localStorage.getItem("pdzancun4") == "false") {
    $.ajax({
      url: "http://39.98.114.107:3000/listContestant4",
      type: "GET",
      contentType: "application/json;charset=UTF-8",
      dataType: "json",
      success: function (response) {
...
const zancun = () => {
  ...
  window.localStorage.setItem("pdzancun4", true);
};
...
const onsubmit = () => {
  ...
  //判断是否重复打分
  // zancun
  if (window.localStorage.getItem("pdfour") == "0") {
    var flag = 0;
    console.log("提交前修改");
    console.log(state.students);
    for (let i = 0; i < state.students.length; i++) {
    ...
    window.localStorage.setItem("pdfour", 1);
    if (flag == 0) {
      ElMessage({
        message: "上传成功",
        type: "success",
