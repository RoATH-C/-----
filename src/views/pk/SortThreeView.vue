<template>
  <div style="width: auto">
    <el-table :data="state.students" border style="width: auto">
      ...
      <el-table-column label="排序" min-width="40%">
        <input
          type="number"
          id="sort"
          v-model="input"
...
onMounted(async () => {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      //挂载的时候获取students的值
      console.log("判断是否打分");
      window.localStorage.setItem("pdthree", response.data);
  ...
  console.log(inputs);
  //先获取各个选手的名字学院
  if (window.localStorage.getItem("pdzancun3") == "false") {
    $.ajax({
      url: "http://39.98.114.107:3000/listContestant3",
      type: "GET",
      contentType: "application/json;charset=UTF-8",
      dataType: "json",
      success: function (response) {
...
const zancun = () => {
  ...
  window.localStorage.setItem("pdzancun3", true);
};
...
const onsubmit = () => {
  ...
  //判断是否重复打分
  // zancun
  if (window.localStorage.getItem("pdthree") == "0") {
    var flag = 0;
    console.log("提交前修改");
    console.log(state.students);
    for (let i = 0; i < state.students.length; i++) {
    ...
    window.localStorage.setItem("pdthree", 1);
    if (flag == 0) {
      ElMessage({
        message: "上传成功",
        type: "success",
